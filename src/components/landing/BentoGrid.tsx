  ...
  return (
    <section ref={ref} className="max-w-7xl mx-auto px-6 py-32">
      ...
      <h2 className="font-serif text-5xl md:text-7xl tracking-tight max-w-3xl mt-6 leading-[1]">
        Built for how teams <em className="text-accent-red">actually</em> live.
      </h2>

      <div className={`mt-20 grid grid-cols-1 md:grid-cols-6 gap-5 transition-opacity duration-700 ${shown ? "opacity-100" : "opacity-0"}`}>
        <Tile className="md:col-span-4 md:row-span-2 bg-ink text-cream min-h-[360px]">
          ...
          <p className="text-cream/60 text-sm mt-6 max-w-sm">Categorised, audited, ready for your accountant.</p>
        </Tile>
        <Tile className="md:col-span-2 bg-accent-red text-cream">
        ...
        <Tile className="md:col-span-2 bg-sage text-cream">
        ...
        <Tile className="md:col-span-3 bg-paper">
        ...
        <Tile className="md:col-span-6 bg-cream hairline">
...
function Eyebrow({ children, tone = "ink" }: { children: React.ReactNode; tone?: "ink" | "cream" }) {
  return (
    <div className={`text-[10px] font-bold uppercase tracking-[0.22em] ${tone === "cream" ? "text-cream/70" : "text-ink-soft"}`}>
      {children}
    </div>
  );
}