...
export function ProviderMapPanel({ offers, onAdd }: { offers: Offer[]; onAdd: (id: string) => void }) {
  ...
  return (
    <section className="rounded-[2rem] overflow-hidden bg-ink border border-ink">
      <div className="grid md:grid-cols-12">
        <div className="md:col-span-4 p-6 md:p-8 bg-ink text-cream">
          <div className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-cream/70 mb-2">Tirana, on the map</div>
          <h2 className="font-serif font-bold text-3xl md:text-4xl leading-tight text-cream">Find perks by neighborhood.</h2>
          <p className="text-cream/80 font-semibold text-sm mt-3">Click a pin to see the offer. Filter by area to plan a route.</p>
            ...
            {neighborhoods.map((n) => (
              <button
                ...
                className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-colors ${
                  active === n ? "bg-cream text-ink border-cream" : "bg-transparent text-cream border-cream/40 hover:border-cream"
                }`}
              >
                {n}
              </button>
